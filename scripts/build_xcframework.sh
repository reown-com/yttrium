...
generate_ffi() {
    ...
    mv target/uniffi-xcframework-staging/*.swift "$swift_package_dir/"
    mv target/uniffi-xcframework-staging/*.h "$swift_package_dir/"
    mv target/uniffi-xcframework-staging/module.modulemap "$swift_package_dir/" || echo "No modulemap to move."
}
