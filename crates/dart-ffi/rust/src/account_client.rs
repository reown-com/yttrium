use super::dart_ffi;
use super::dart_ffi::{FFIAccountClientConfig, FFIError};
use crate::dart_ffi::{
        ...
        let account_client = match signer {
            SignerType::Native => {
                let sign_fn = Box::new(move |message: String| {
                    let signer_service =
                    dart_ffi::NativeSignerFFI::new(signer_id.clone());
                    let sign = signer_service.sign(message);
                    match sign {
                        dart_ffi::FFIStringResult::Ok(signed_message) => {
                            Ok(signed_message)
                        }
            ...
            SignerType::PrivateKey => {
                let private_key_fn = Box::new(move || {
                    let private_key_service =
                        dart_ffi::PrivateKeySignerFFI::new(signer_id.clone());
                    let private_key = private_key_service.private_key();
                    match private_key {
                        dart_ffi::FFIStringResult::Ok(private_key) => {
                            Ok(private_key)
                        }
                        dart_ffi::FFIStringResult::Err(error) => {
                            Err(YttriumError { message: error })
...
impl From<dart_ffi::FFITransaction> for Transaction {
    fn from(transaction: dart_ffi::FFITransaction) -> Self {
