...
impl StacksProvider {
    pub async fn stacks_transactions(
        ...
        let response: serde_json::Value = self.client.request("stacks_transactions", tx_hex).await?;
    ...
    pub async fn estimate_fee(&self) -> TransportResult<u64> {
        ...
        let response: serde_json::Value = self.client.request("get_fee_rate", ()).await?;
        ...
        let fee_rate = response
            .get("fee_rate")
            .and_then(|v| v.as_u64())
            .unwrap_or(180); // Default fallback fee
    ...
    pub async fn get_account_balance(&self, address: &str) -> TransportResult<u64> {
        // Query the STX balance for the given address
        let response: serde_json::Value = self.client.request("get_account", (address,)).await?;
