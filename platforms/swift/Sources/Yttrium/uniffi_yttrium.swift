...
public protocol SignClientProtocol: AnyObject {
    ...
    func approve(pairing: SessionProposalFfi) async throws 
...
#endif
    public func uniffiClonePointer() -> UnsafeMutableRawPointer {
        return try! rustCall { uniffi_uniffi_yttrium_fn_clone_signclient(self.pointer, $0) }
    }
public convenience init(relayUrl: String, projectId: String, clientId: String) {
    let pointer =
        try! rustCall() {
    uniffi_uniffi_yttrium_fn_constructor_signclient_new(
    ...
    deinit {
        ...
        try! rustCall { uniffi_uniffi_yttrium_fn_free_signclient(pointer, $0) }
    }

    

    
open func approve(pairing: SessionProposalFfi)async throws   {
    return
        try  await uniffiRustCallAsync(
            rustFutureFunc: {
                uniffi_uniffi_yttrium_fn_method_signclient_approve(
                    self.uniffiClonePointer(),
                    FfiConverterTypeSessionProposalFfi_lower(pairing)
                )
            },
            pollFunc: ffi_uniffi_yttrium_rust_future_poll_void,
            completeFunc: ffi_uniffi_yttrium_rust_future_complete_void,
            freeFunc: ffi_uniffi_yttrium_rust_future_free_void,
            liftFunc: { $0 },
...
open func pair(uri: String)async throws  -> SessionProposalFfi  {
    return
        try  await uniffiRustCallAsync(
            rustFutureFunc: {
                uniffi_uniffi_yttrium_fn_method_signclient_pair(
                    self.uniffiClonePointer(),
...
private let initializationResult: InitializationResult = {
    ...
    if (uniffi_uniffi_yttrium_checksum_method_signclient_approve() != 63708) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_uniffi_yttrium_checksum_method_signclient_pair() != 65176) {
        return InitializationResult.apiChecksumMismatch
    }
    ...
    if (uniffi_uniffi_yttrium_checksum_constructor_signclient_new() != 2380) {
        return InitializationResult.apiChecksumMismatch
    }
